...
<?php	 
	...
	if (isset($_POST['login'])){
		...
		if(empty($uemail) || empty($upassword)){
			header("Location: ../search/search.php?login=incorrectfirst");
			exit();
		}else{
			...
			<?php
				...
				if($queryresult < 1) {
					header("Location: ../search/search.php?login=incorrectsecond");
					exit();
				}else{
	...
	{
		header("Location: ../search/search.php?signup=error");
		exit();
	}	
?>
