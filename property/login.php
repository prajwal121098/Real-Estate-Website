...
<?php	 
	...
	if (isset($_POST['login'])){
		...
		if(empty($uemail) || empty($upassword)){
			header("Location: ../property/property.php?login=incorrectfirst");
			exit();
		}else{
			...
			<?php
				...
				if($queryresult < 1) {
					header("Location: ../property/property.php?login=incorrectsecond");
					exit();
				}else{
	...
	{
		header("Location: ../property/property.php?signup=error");
		exit();
	}	
?>
